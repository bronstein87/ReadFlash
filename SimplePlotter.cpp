...
_fastcall SimplePlotter::SimplePlotter() : Title("IKI"), SeriesColor(clBlue),
	ShowPoints(true), ShowLines(false), PointSize(3), PointerStyle(psCircle) {
...
void SimplePlotter::SaveChart(TChart* Chart, AnsiString name,
...
void SimplePlotter::AddPoint(TChart* Chart, DWORD SeriesIndex, double X,
	double Y, TColor Color) {
	if (Chart->SeriesCount() <= SeriesIndex) {
		for (int i = Chart->SeriesCount(); i <= SeriesIndex; i++) {
			Chart->AddSeries(new TLineSeries(Chart));
			SetSeriesOptions(dynamic_cast<TLineSeries*>(Chart->Series[i]));
...
void SimplePlotter::AddArray(TChart* Chart, DWORD SeriesIndex,
	const double* ArrayX, const double* ArrayY, DWORD ArraySize) {
	if (Chart->SeriesCount() <= SeriesIndex) {
		Chart->AddSeries(new TLineSeries(Chart));
		SetSeriesOptions(dynamic_cast<TLineSeries*>

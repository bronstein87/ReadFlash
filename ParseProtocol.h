	...
	namespace parse_prot {
		...
		void readBOKZM60Mil (ifstream& in, vector<CadrInfo>& cadrInfoVec,
			ProtHandler handle)
			{
				const string mshior = "КС:	..18";
				const string loc = "КС:	..80";
		...
		void readBOKZMMil(ifstream& in, vector<CadrInfo>& cadrInfoVec,
			...
			while (getline(in, line)) {
				...
				for (int i = 0; i < skipBeforeData; i++) {
					 getline(in, line); // это нужно оставить если протокол полный, остальное - убрать
					 //tmp
			...
			try {
				...
				while (getline(in, line)) {
					...
					if (line.find("Состав ДТМИ:") != string::npos) {
						...
						{
							getline(in, line);
							if (line.find("ошибка") != string::npos)
							{
								skipDtmi = true;
								break;
							}
...
//							!= string::npos) {
						...
						if (findLine(in, "Состав МШИ ОР") != string::npos) {
							...
							in.seekg(pos);
						if (findLine(in, "Кватернион ориентации, Qо") != string::npos
						&& cadrInfo.DeviceInfo == "SLEZH")
						{
							for (int i = 0; i < 4; i++) {
								getline(in, line);
	...
	void readBOKZ60DORKondor(ifstream& in, vector<CadrInfo>& cadrInfoVec,
		...
		try {
			...
			while (getline(in, line)) {
				bool error = false;
				TColor PointColor = clGreen;
				if (line.find("ДОР;Ответ;Успешно;") != string::npos) {
					CadrInfo cadrInfo;
					cadrInfo.ImageHeight = 512;
					cadrInfo.ImageWidth = 512;
					getline(in, line);
...
//					if (findWord(in, "пикселей") != string::npos) {
//						int number;
//						in >> number;
//						if (number == 0)
//							continue;
...
//						throw logic_error(errorMessage);
					...
					// ищем начало массива лок и фрагментов
					if (findLine(in, "Массив локализованных объектов на 1-ом кадре")
						!= string::npos) {
						ObjectsInfo objInfo;
						WindowsInfo winInfo;
...
//
					...
					if (findLine(in,
							"         Массив координат центров фрагментов                      ")
							!= string::npos) {
							for (int i = 0; i < maxCountLocObj; i++) {
								getline(in, line);
								splittedStr = split(line, " ");
					...
					if (findLine(in,
							"         Массив cредней яркости и с.к.о. сигнала во фрагментах    ")
							!= string::npos) {
					...
					if (findLine(in,
							"         Порог локализации во фрагментах                          ")
							!= string::npos) {
							for (int i = 0; i < cadrInfo.WindowsList.size(); i++) {
								getline(in, line);
								splittedStr = split(line, " ");
					...
					if (findLine(in,
							"         Количество объектов во фрагментах                        ")
							!= string::npos) {
							for (int i = 0; i < cadrInfo.WindowsList.size(); i++) {
								getline(in, line);
								splittedStr = split(line, " ");
					...
					if (findLine(in, "         Кватернион ориентации. Qо     ")
						!= string::npos) {
					...
					if (findWord(in, "КМОП-матрицы") != string::npos) {
						in >> cadrInfo.MatrixTemp;
					}
					else
						throw logic_error(errorMessage);
				...
				if (!cadrInfoVec.empty()) {
					...
					if (NeedNextFile) {
						...
						if (readLastData) {
							...
							if (findLine(in, "Код состояния 2")
								!= string::npos) {
								getline(in, line);
								vector <string> splittedStr = split(line, "\t");

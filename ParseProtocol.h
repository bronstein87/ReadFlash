...
namespace parse_prot
{
   ...
   static  string arrStatErrorEng[MAX_STAT] = {
	...
	void readBOKZ60LocProtocol(ifstream& in, vector <CadrInfo>& cadrInfoVec, ProtHandler handle, TDateTime& startDate)
{
	string line;
	...
	void readBOKZMFProtocol(ifstream& in, vector <CadrInfo>& cadrInfoVec, ProtHandler handler, TDateTime& dt)
{
	...
	void readBOKZM2LocProtocol(ifstream& in, vector <CadrInfo>& cadrInfoVec, ProtHandler handler, TDateTime& dt)
	{
		...
		{
			TColor pointColor = clBlue;
			if (contains(line, "Расшифровка;МПИ;;ДТМИ Лок;") && !contains(line, "Таблица"))
			{
				CadrInfo cadrInfo;
				cadrInfo.ImageHeight = 512;
				cadrInfo.ImageWidth = 512;

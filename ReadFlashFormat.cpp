...
void ConvertDataSLEZH(struct DataSLEZH data, struct CadrInfo &mCadr) {
	...
	if ((!data.res_stat) && (data.NumDet)) {
		...
		mCadr.MeanErrorXY = data.m_cur;
		// sqrtm((SumX+SumY)/(double)(2*data.NumDet-1));
	}
...
void PrintDataNO(FILE *ftxt, struct DataNO data) {
	...
	fprintf(ftxt, "Априорный кватернион: %f; %f; %f; %f;\n", data.QuatBoart[0],
		data.QuatBoart[1], data.QuatBoart[2], data.QuatBoart[3]);
	...
	for (int i = 0; i < data.NumL[0]; i++) {
		fprintf(ftxt, "%4d %10.4f %10.4f %10.2f %8.0f\n", i + 1,
			data.RsLocT[0][i][0], data.RsLocT[0][i][1], data.RsLocT[0][i][2],
			data.RsLocT[0][i][3]);
	}
	fprintf(ftxt, "RsLocT[1]\n");
	for (int i = 0; i < data.NumL[1]; i++) {
		fprintf(ftxt, "%4d %10.4f %10.4f %10.2f %8.0f\n", i + 1,
			data.RsLocT[1][i][0], data.RsLocT[1][i][1], data.RsLocT[1][i][2],
			data.RsLocT[1][i][3]);
	}
	...
	for (int i = 0; i < MaxDetHO; i++) {
		fprintf(ftxt, "%4d %10.4f %10.4f %10.2f %8.0f\n", i + 1,
			data.RsLoc[i][0], data.RsLoc[i][1], data.RsLoc[i][2],
			data.RsLoc[i][3]);
	}
	...
	for (int i = 0; i < MaxDetHO; i++) {
		fprintf(ftxt, "%4d %4d %4d\n", i + 1, data.MasRes[i][0],
			data.MasRes[i][1]); // НЕ меняем местами
	}

	fprintf(ftxt, "Coord\n");
	for (int i = 0; i < MaxDetHO; i++) {
		fprintf(ftxt, "%4d %12.8f %12.8f %12.8f %10.2f\n", i + 1,
			data.Coord[i][0], data.Coord[i][1], data.Coord[i][2],
			data.Coord[i][3]);
	}
	...
	for (int i = 0; i < MaxDetHO; i++) {
		fprintf(ftxt, "%4d %12.8f %12.8f %12.8f %8.0f\n", i + 1,
			data.D_cos[i][0], data.D_cos[i][1], data.D_cos[i][2],
			data.D_cos[i][3]);
	}
	...
	fprintf(ftxt, "%16.8f %16.8f %16.8f\n", data.M_ornt[0][0],
		data.M_ornt[0][1], data.M_ornt[0][2]);
	fprintf(ftxt, "%16.8f %16.8f %16.8f\n", data.M_ornt[1][0],
		data.M_ornt[1][1], data.M_ornt[1][2]);
	fprintf(ftxt, "%16.8f %16.8f %16.8f\n", data.M_ornt[2][0],
		data.M_ornt[2][1], data.M_ornt[2][2]);
	...
	fprintf(ftxt, "Угловая скорость, угл.с./с: %f %f %f\n", data.Wop[0]*RTS,
		data.Wop[1]*RTS, data.Wop[2]*RTS);
	fprintf(ftxt, "Эпоха наблюдения: %12.8f\n", data.EPOCH);

	fprintf(ftxt, "Линейные скорости, км/с:\n");
	fprintf(ftxt, "%f %f %f\n", GetFloat(data.Vline[0]),
		GetFloat(data.Vline[1]), GetFloat(data.Vline[2]));
	fprintf(ftxt, "//----------------------------------------------//\n");
}
...
void PrintDataSLEZH(FILE *ftxt, struct DataSLEZH data) {
	...
	fprintf(ftxt, "%16.8f %16.8f %16.8f\n", data.M_prog[0][0],
		data.M_prog[0][1], data.M_prog[0][2]);
	fprintf(ftxt, "%16.8f %16.8f %16.8f\n", data.M_prog[1][0],
		data.M_prog[1][1], data.M_prog[1][2]);
	fprintf(ftxt, "%16.8f %16.8f %16.8f\n", data.M_prog[2][0],
		data.M_prog[2][1], data.M_prog[2][2]);
	...
	for (int i = 0; i < MaxOrnt; i++) {
		fprintf(ftxt, "%4d %10.4f %10.4f %8.0f %8.2f\n", i + 1, data.XYc[i][0],
			data.XYc[i][1], data.XYc[i][2], data.XYc[i][3]);
	}
	...
	for (int i = 0; i < 8; i++) {
		fprintf(ftxt, "%4d %6d %6d\n", i + 1, data.TabTakeAway[i][0],
			data.TabTakeAway[i][1]); // НЕ меняем местами
	}
	fprintf(ftxt, "PfragXY0\n");
	for (int i = 0; i < MaxOrnt; i++) {
		fprintf(ftxt, "%4d %6d %6d\n", i + 1, data.PfragXY0[i][0],
			data.PfragXY0[i][1]); // НЕ меняем местами
	}

	fprintf(ftxt, "PfragXY\n");
	for (int i = 0; i < MaxOrnt; i++) {
		fprintf(ftxt, "%4d %6d %6d\n", i + 1, data.PfragXY[i][0],
			data.PfragXY[i][1]); // НЕ меняем местами
	}

	fprintf(ftxt, "ZipXY\n");
	for (int i = 0; i < MaxOrnt; i++) {
		fprintf(ftxt, "%4d %6d %6d\n", i + 1, data.ZipXY[i][0],
			data.ZipXY[i][1]); // НЕ меняем местами
	}

	fprintf(ftxt, "FragYX\n");
	for (int i = 0; i < MaxOrnt; i++) {
		fprintf(ftxt, "%4d %6d %6d\n", i + 1, data.FragYX[i][0],
			data.FragYX[i][1]); // НЕ меняем местами
	}
	...
	for (int i = 0; i < data.NumLoc; i++) {
		fprintf(ftxt, "%4d %10.4f %10.4f %10.2f %8.0f\n", i + 1,
			data.RsLocT[i][0], data.RsLocT[i][1], data.RsLocT[i][2],
			data.RsLocT[i][3]);
	}
	...
	for (int i = 0; i < MaxOrnt; i++) {
		fprintf(ftxt, "%4d %4d %4d\n", i + 1, data.MasRes[i][1],
			data.MasRes[i][0]); // меняем местами
	}

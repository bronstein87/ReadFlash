...
// ёлианска€ дата 2000 года 12 часов 1 €нвар€  = 2451545.0;
	double JD = 0;
// прибавл€ем число суток за прошедшие  года (не вкл текущий год)
...
//прибавл€ем 29 феврал€, если оно было в текущем году
	if ((stDateTime->Year % 4 == 0) && (stDateTime->Month >= 3)) JD += 1.;

	//считаем число дней в текущем году, не вкл текущий мес€ц
	for (i = 1; i < stDateTime->Month; i++) JD += (double)mes[i-1];
	JD += stDateTime->Day - 1;  // прибавл€ем число дней в текущем мес€це
	JD  = JD - 0.5;

...
struct CadrInfo
{
	...
	CountLocalObj(0), CountDeterObj(0), SizeStarsList(0), SizeWindowsList(0), SizeObjectsList(0),
	 ...
	 AnglesDiff(),  OmegaOrient(), OmegaModel(), //углова€ скорость, прогнозируема€ матрица ориентации
	 OmegaDiff(), AxesDiff() { }

	double Time, TimeBoard;
	std::string DeviceInfo;
	int FrameNumber;
	bool IsBinary, IsReverse;   //признак бинировани€, признак реверса изображени€
	int ResolutionACP;
	bool IsOrient; //признак определени€ ориентации
	unsigned short CountPixFilter;       //число пикселей выше порога
	unsigned short ImageWidth, ImageHeight;  //ширина и высота изображени€
	double SizePixel;
	unsigned short StatOrient;  //статус решени€ задачи (0 - успешно)
	...
	double OmegaOrient[3], OmegaModel[3], MatrixProg[3][3]; //углова€ скорость, прогнозируема€ матрица ориентации
	double AxesDiff[3], AnglesDiff[3], OmegaDiff[3];  //ошибки определени€ ориентации и угл. скорости
	double SunAngle, MoonAngle;  //угол опт. оси к —олнцу и Ћуне
	double MatrixTemp;      //температура  ћќѕ-матрицы
	double MeanErrorX, MeanErrorY, MeanErrorXY; //остаточные рассогласовани€
};
